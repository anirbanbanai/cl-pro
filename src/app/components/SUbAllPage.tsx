...
import { LuCircleDotDashed, LuSearch } from "react-icons/lu";
...
const SubAllPage = () => {
  ...
  const [whiteCustomizationSelect, setWhiteCustomizationSelect] = useState<
    string | undefined
  >(undefined);
  ...
  console.log(whiteCustomizationSelect);
    ...
    <section className="grid grid-cols-12 gap-3 px-10 py-10 h-svh overflow-hidden">
      ...
      <div className="col-span-4">
        ...
        {selectedFabric === "White" && (
          <div className="mt-2">
            <p>White Customization</p>
            {
              <ul className="grid grid-cols-9 gap-3">
                {whiteCustom.map((item, index) => (
                  <div
                    key={index}
                    className=" border flex flex-col items-center bg-gray-700 text-white cursor-pointer rounded-lg p-2"
                    onClick={() => setWhiteCustomizationSelect(item.src)}
                  >
                    <Image src={item.src} width={33} alt="" height={44}/>
